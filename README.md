...
Создание задачи и обновление задачи каждое пишет в `task_history` в той
же транзакции, что и сам insert/update (`TaskService.CreateTask`,
`TaskService.UpdateTask`): история не должна разойтись с фактическим
состоянием задачи, а `GET /tasks/{id}/history` — быть полным журналом
с момента создания, а не только с первого изменения.
